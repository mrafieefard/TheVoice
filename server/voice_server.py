    ...
    def process_voice_data(self, data: bytes, addr: Tuple[str, int]) -> Optional[bytes]:
        ...
        try:
            ...
            if data.startswith(b"PING"):
                ...
                # Comment out disk I/O to reduce latency
                # append_pcm_to_wav(
                #     "voice_data.wav", 
                #     voice_data,
                #     sample_rate=22050,  # Updated to match client
                #     channels=1,         # Updated to match client (mono)
                #     sample_width=2,
                # )
...
async def voice_server(host: str = "localhost", port: int = 8888):
    """
